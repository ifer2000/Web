...
function sortByDate(data) {
    return [...data].sort((a, b) => a["생산일자"] - b["생산일자"]);
}
...
function applyFilters() {
    ...
    const dateFiltered = DUMMY_DATA.filter(item => {
        ...
        const iy = Number(dateStr.substring(0, 4));
        const im = Number(dateStr.substring(4, 6));
        const id = Number(dateStr.substring(6, 8));
...
function updateChart() {
    ...
    const targetProd = ValuesByKey(sortedData, "목표 생산량");
    const predictedProd = ValuesByKey(sortedData, "예측 생산량");
...
function initializeChart() {
    ...
    const ctx = document.getElementById('chartli');
        ...
        datasets: [
          {
            label: '목표 생산량',
            data: targetProd,
            borderColor: 'rgba(80, 123, 209, 1)',
            backgroundColor: 'rgba(80, 123, 209, 0.5)',
            tension: 0,
            borderWidth: 3,
            fill: false,
            pointRadius: 5,
            pointBackgroundColor: 'rgba(195, 215, 255, 1)',
          },
          {
            label: '예측 생산량',
            data: predictedProd,
            borderColor: 'rgba(213, 42, 79, 1)',
            backgroundColor: 'rgba(213, 42, 79, 0.5)',
            tension: 0,
            borderWidth: 3,
            fill: false,
            pointRadius: 5,
            pointBackgroundColor: 'rgba(255, 199, 199, 1)',
          }
        ]
      },
      options: {
        ...
        plugins: {
          ...
          title: { display: true, text: '생산량 목표 및 예측 그래프', font:{size: 30}, color: '#000000ff'},
          tooltip: {
            enabled: true,
